...
このプロジェクトでは、以下の 2 つを明確に分けて管理します。
...
運用上の入口は `ops add` `ops update` `ops repair` に寄せ、内部では既存の `council` `meetings` `hierarchy` `quality` コマンドを組み合わせる構成にします。つまり、CLI の見せ方は運用単位、実装の責務分離はデータ種別単位です。

また、`fixtures/` はローカル開発専用です。GitHub Actions などの本番運用では live fetch を前提にし、コミット対象は `data/` 配下の成果物に限定します。
...
- 本番運用では一覧ページを live fetch し、差分がある詳細ページだけ読む
- 開発中は保存済み HTML を優先して読む
- パーサは「HTTP から取ったか、ローカルから読んだか」を意識しない

現時点では、行政サイト側の `ETag` / `Last-Modified` は前提にしません。代わりに、一覧ページから得られる ID やリンク群を既存 `data/` と比較し、変化がない会議の詳細ページ再取得を避けます。
